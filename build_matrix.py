...
if __name__ == "__main__":
    ...
    if build_H:
        ...
        if not (phi_y is None):
            non_interacting_H = build_non_interacting_H(Nx, Ny, n=n,phase_shift_y= 0.625 * 2 * np.pi ,phase_shift_x= 2 * np.pi * phi_y, multi_process=multi_process,max_workers=max_workers,multiprocess_func=multiprocess_func)
            os.makedirs(save_path, exist_ok=True)
            print("Saving H")
